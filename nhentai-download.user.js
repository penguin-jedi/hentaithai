...
$(document).ready(async () => {
  ...
  const downloadGallery = (indexOffsetStart, indexOffsetEnd) => async () => {
    ...
    for (let i = 0; i < imgPages.length; i+= PER_PAGE) {
      const pages = i+PER_PAGE <= imgPages.length ? Array(PER_PAGE) : Array(imgPages.length - i);
      for (let j = 0; j < PER_PAGE; j++) {
        pages[j] = imgPages[i + j];
      }
      const res = await Promise.all(pages.map(async (url, index) => {
        const res = await httpGet(url);
        const imgSrc = scrapImgSrc(res.responseText);
        imageContents[i + index] = await httpGet(imgSrc);
      }));
    }
