...
// @author       penguin-jedi
// @homepage     https://github.com/penguin-jedi/kemono
// @downloadURL  https://github.com/penguin-jedi/kemono/raw/release/kemono-download.user.js
// @updateURL    https://github.com/penguin-jedi/kemono/raw/release/kemono-download.user.js
// @supportURL   https://github.com/penguin-jedi/kemono/issues
// @include      /^https?:\/\/(www\.)?kemono\.party\/(fanbox|patreon)\/user\/\d+\/post\/\d+/
...
$(document).ready(async () => {
  ...
  const downloadGallery = (indexOffsetStart, indexOffsetEnd) => async () => {
    ...
    const title = $("h1.post__title > span").map((_index, element) => element.innerHTML).get().join('') || $('title').html();
    const imgSrcs = $("a.fileThumb").get().map((element) => element.href).filter((_value, index, array) => index >= indexOffsetStart && index < array.length - indexOffsetEnd);

    const imageContents = Array(imgSrcs.length);
    for (let i = 0; i < imgSrcs.length; i+= PER_PAGE) {
      const pages = i+PER_PAGE <= imgSrcs.length ? Array(PER_PAGE) : Array(imgSrcs.length - i);
      for (let j = 0; j < pages.length; j++) {
        pages[j] = imgSrcs[i + j];
      }
      const res = await Promise.all(pages.map(async (url, index) => {
        imageContents[i + index] = await httpGet(url, headers);
      }));
    }
