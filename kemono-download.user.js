...
$(document).ready(async () => {
  const CONCURRENT = 8;
  const RETRY_DELAY_MILLISECOND = 512;
  ...
  const downloadGallery = (indexOffsetStart, indexOffsetEnd) => async () => {
    ...
    const retryableTask = async (taskId) => {
      for (let element = imgQueue.shift(); !!element; element = imgQueue.shift()) {
        ...
        await retryableJob(attempt, (error) => {
          const imageIndex = `[${taskId}]`;
          // console.log('imageIndex', imageIndex);
          // console.error(imageIndex, error.stack);
        }, RETRY_MAX_COUNT);
      }
    };
    ...
    for (let i = 0; i < imageContents.length; i++) {
      ...
      const almostFileName = imageContent.finalUrl.substring(imageContent.finalUrl.lastIndexOf('/') + 1);
      const fileName = almostFileName.split('?f=')[1];
  ...
  $('div.post__thumbnail').css('position', 'relative');
  $('div.post__thumbnail').append((index) => $(`
    <div style="position: absolute; font-size: 100px; background-color: rgba(0,0,200,0.3); top: 0px; color: white; padding: 8px;">${index + 1}</div>
  `));
