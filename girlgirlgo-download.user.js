// ==UserScript==
// @name         girlgirlgo-download
// @version      0.1
// @description  insert download gallery button
// @author       penguin-jedi
// @homepage     https://github.com/penguin-jedi/hentaithai
// @downloadURL  https://github.com/penguin-jedi/hentaithai/raw/release/girlgirlgo-download.user.js
// @updateURL    https://github.com/penguin-jedi/hentaithai/raw/release/girlgirlgo-download.user.js
// @supportURL   https://github.com/penguin-jedi/hentaithai/issues
// @match        *://*.girlgirlgo.org/a/*
...
$j(document).ready(async () => {
  ...
  const fileExtention = (contentType) => {
    ...
    if (!contentTypeMap[contentType]) console.warn('UNKNOWN_CONTENT_TYPE', contentType);
  ...
  const downloadGallery = (indexOffsetStart, indexOffsetEnd) => async () => {
    ...
    const title = $j("h1.post-title > a").text() || $j('title').html();
    const imgSrcs = $j("div.figure-link-w > a.figure-link").get().map((element) => $j(element).prop('href')).filter((_value, index, array) => index >= indexOffsetStart && index < array.length - indexOffsetEnd);
    ...
    const galleryId = window.location.pathname.split('/').at(-1);
    const zip = new JSZip();
